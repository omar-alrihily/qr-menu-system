...
export default async function PublicMenuPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    // 1. تعريف متغيرات الألوان القادمة من قاعدة البيانات
    const themeStyles = {
        ...
        <div className="max-w-2xl mx-auto px-5 -mt-20 relative z-30">
          <div className="bg-white/90 backdrop-blur-2xl rounded-[2.5rem] shadow-xl shadow-gray-200/50 p-6 border border-white/50">
            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 bg-white rounded-[2rem] p-1.5 shadow-2xl -mt-20 mb-4 border-4 border-[var(--bg-color)] relative">
                <Image 
              ...
              <div className="flex flex-wrap justify-center items-center gap-4 text-gray-500 text-[11px] font-bold uppercase tracking-wide">
                <span className="flex items-center gap-1.5">
                  <MapPin size={14} style={{ color: 'var(--primary-color)' }} /> الرياض، المملكة العربية السعودية
                </span>
              </div>
            ...
            {categories.map((cat: any) => (
              <a 
                ...
                style={{ 
                   ...
                   borderColor: 'rgba(0,0,0,0.05)' // حدود خفيفة جداً افتراضياً
                }}
                className="px-6 py-2.5 rounded-2xl text-[13px] font-black transition-all whitespace-nowrap border-2 bg-white shadow-sm hover:border-[var(--primary-color)] hover:scale-105 active:scale-95"
              >
                {cat.name_ar}
              </a>
            ))}
