...
import { Plus, Minus, ShoppingBag, X, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
              ...
              <div className="grid gap-5">
                {catProducts.map((product: any) => {
                  ...
                  return (
                    <div 
                      key={product._id} 
                      className="group relative bg-white rounded-[2rem] p-4 flex gap-4 border border-gray-100 shadow-sm hover:shadow-xl hover:border-orange-100 transition-all duration-300 active:scale-[0.98]"
                    >
                      <div className="flex-1 flex flex-col justify-between py-1 pr-1">
                        <div>
                          <h3 className="font-black text-gray-900 text-lg mb-1 leading-tight group-hover:text-orange-600 transition-colors">
                            {product.name_ar}
                          </h3>
                        ...
                        <div className="flex items-center justify-between mt-4">
                          <div className="flex flex-col">
                             ...
                             <span className="text-xl font-black text-gray-900">
                               {product.price} <small className="text-[11px] font-bold text-orange-500 mr-0.5">ر.س</small>
                             </span>
                          </div>
                          ...
                          {cartItem ? (
                            <div className="flex items-center gap-3 bg-gray-900 rounded-2xl p-1.5 shadow-lg animate-in zoom-in-90 duration-300">
                              ...
                              <button 
                                ...
                                className="w-8 h-8 flex items-center justify-center bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors shadow-inner"
                              >
                            ...
                            <button 
                              ...
                              className="h-12 px-6 bg-gray-50 text-gray-900 rounded-2xl flex items-center justify-center font-black text-sm gap-2 hover:bg-orange-500 hover:text-white transition-all shadow-sm border border-gray-100"
                            >
                              إضافة
                              <Plus size={16} strokeWidth={3} />
                            </button>
...
{/* --- Floating Action Button (السلة العائمة الحقيقية) --- */}
{cartCount > 0 && !isCartOpen && (
  <div className="fixed bottom-10 left-0 right-0 flex justify-center px-6 z-50 animate-in fade-in slide-in-from-bottom-10">
    <button 
      onClick={() => setIsCartOpen(true)} // هذا السطر هو مفتاح الحل
    ...
    >
      <div className="flex items-center gap-4">
        <div className="relative">
          <div className="w-12 h-12 bg-orange-500 rounded-2xl flex items-center justify-center shadow-lg rotate-3 group-hover:rotate-0 transition-transform">
          ...
          <div className="relative bg-[#F8F9FA] rounded-t-[3rem] p-8 w-full max-w-2xl mx-auto max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-500">
            ...
            <div className="flex items-center justify-between mb-8 mt-2">
              ...
              <button 
                ...
                className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-gray-400 shadow-sm border border-gray-100"
              >
                <X size={20} />
              </button>
            </div>
              ...
              {cart.map((item) => (
                <div key={item._id} className="flex items-center gap-4 p-4 bg-white rounded-[1.5rem] border border-gray-100 shadow-sm">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-orange-600 font-black text-sm">{item.price} ر.س</p>
                  </div>
                  <div className="flex items-center gap-3 bg-gray-50 rounded-2xl p-1.5 border border-gray-100">
                    ...
                    <button onClick={() => addToCart(item)} className="w-8 h-8 flex items-center justify-center text-orange-500">
                      <Plus size={16} />
                    </button>
                  </div>
                </div>
            ...
            <div className="mt-6 pt-6 bg-white -mx-8 px-8 pb-4 border-t border-gray-100 shadow-[0_-20px_40px_rgba(0,0,0,0.02)]">
              <div className="flex justify-between items-end mb-6">
                <div>
                  <p className="text-xs text-gray-400 font-bold uppercase mb-1 tracking-tighter">إجمالي المبلغ</p>
                  <p className="text-4xl font-black text-gray-900">
                    {cartTotal.toFixed(2)} 
                    <span className="text-sm font-bold text-orange-500 mr-2">ر.س</span>
                  </p>
                </div>
                <p className="text-[10px] text-gray-400 font-medium max-w-[100px] text-left">الأسعار تشمل ضريبة القيمة المضافة</p>
              </div>
